...
MainWidget::MainWidget()
{
    ...
    */
    ...
    HBoxLayout->addWidget(Right_Table_Box,1);
}
...
QGroupBox * MainWidget::createStuMess(){
    ...
    TableWidget->setSelectionBehavior(QAbstractItemView::SelectRows);//选择模式-整行选中
    ...
    AutoHBoxLayout->addWidget(TableWidget);//向水平布局中添加表格控件
    box->setLayout(AutoHBoxLayout);//将布局管理器添加到box容器控件
...
QGroupBox * MainWidget::createMenu(){
    ...
    FindStuEdit->setPlaceholderText("输入学生姓名查找...");//提示信息
    FindStuEdit->setClearButtonEnabled(true);//显示清除按钮
    ...
    VBoxLayout->addWidget(ListWidget,4);//向布局中添加控件
    VBoxLayout->addLayout(Buts,2);//向布局中添加子布局

    box->setLayout(VBoxLayout);//将布局管理器添加到box容器控件
...
void MainWidget::AddStuBox(){
    ...
    messBox->exec();//显示一个模式对话框，并且锁住程序直到用户关闭该对话框为止
}
    ...
    if(items.count() > 0){
        ...
        if(result == QMessageBox::Yes){
            ...
            while (!readDataStr.atEnd()) {
                ...
                if(ID != items.at(0)->text()){//将除目标学生外，其它学生的信息拷贝到一个临时文件中
            ...
            flushTable();//刷新表格中的内容
            ListWidget->clear();//清理所有的QListWidgetItem项，释放所有项的QListWidgetItem对象内存
        }
    }else{
    ...
    if(count>0){
        for(qint32 i=0;i<count;i++){
            ...
            if(name == FindStuEdit->text()){
                findSuccess = true;
                TableWidget->selectRow(i);//选择i行
                flushListWidget(i);//更新列表中显示的学生信息
                break;
            }
        }
...
void MainWidget::changeStuMess(int row)
{
    ...
    while (!readDataStr.atEnd()) {
        readDataStr >> ID >> name >> age >> sex >> chinese_score >> math_score >> english_score;
        if(name != stuName){//将除目标学生外，其它学生的信息拷贝到一个临时文件中
            writeDataStr << ID << name << age << sex << chinese_score << math_score << english_score;
        }else{
